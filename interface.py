...
class requestsAPP(customtkinter.CTk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Mon application GPT Perso")
        ...
        self.slider_token.set(self.user_max_token)

        self.token_label = customtkinter.CTkLabel(master=self.token_frame, text=f"Max Token : {self.user_max_token}")
        self.token_label.grid()

        # Partie condition 
        self.condition_frame = customtkinter.CTkFrame(self, width=800, corner_radius=0) 
        self.condition_frame.grid(row=2, column=0)
        
        self.user_condition = customtkinter.CTkEntry(master=self.condition_frame, height=50, width=300)
        self.user_condition.grid()

        self.condition_button = customtkinter.CTkButton(master=self.condition_frame, text="Apply", command=self.apply_condition)
        self.condition_button.grid()
    ...
    def apply_condition(self):
        c = self.user_condition.get()
        self.user_condition_text = c
        # print(self.user_condition_text)
        self.historique_conversation = [{'role': 'system', 'content' : self.user_condition_text}]
    ...
    def update_max_token(self, value):
        ...
        self.token_label.configure(text=f"Token : {self.user_max_token}")
